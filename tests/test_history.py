...
def test_record_and_read_back():
    history.record_prediction("healthy", 0.93, {"healthy": 0.93, "cssvd": 0.04, "anthracnose": 0.03}, b"fake-image-bytes")

    items, total = history.list_predictions(limit=20, offset=0)
...
def test_retention_evicts_oldest_and_its_image_file():
    for i in range(history.RETENTION_LIMIT + 1):
        history.record_prediction("healthy", 0.9, {"healthy": 0.9}, f"image-{i}".encode())

    items, total = history.list_predictions(limit=1000, offset=0)
...
def test_image_file_written_and_path_matches_stored_reference():
    history.record_prediction("cssvd", 0.7, {"cssvd": 0.7}, b"some-bytes")
...
def test_ordering_is_newest_first():
    ...
    history.record_prediction("cssvd", 0.8, {"cssvd": 0.8})
    history.record_prediction("anthracnose", 0.7, {"anthracnose": 0.7})

    items, _ = history.list_predictions(limit=10, offset=0)
...
def test_pagination_returns_only_requested_page():
    ...
    page, total = history.list_predictions(limit=2, offset=0)
    assert total == 5
    assert len(page) == 2

    next_page, _ = history.list_predictions(limit=2, offset=2)
